...
export default function EmployeeExtract() {
  ...
  // Count today's castings for sequence
  const { data: todayCastingCount } = useQuery({
    queryKey: ['today_casting_count'],
    queryFn: async () => {
  ...
  const flaskCode = generateFlaskCode((todayCastingCount ?? 0) + 1);
  ...
  if (success && metal) {
    return (
      <div className="flex flex-col items-center justify-center p-8 text-center min-h-[60vh]">
        ...
        <h2 className="text-xl font-bold mb-1">Extraction Complete</h2>
        <p className="text-muted-foreground mb-1">Flask {flaskCode}</p>
  ...
  return (
    <div className="p-4 max-w-lg mx-auto">
      ...
      {/* Flask code */}
      <div className="rounded-lg bg-muted px-4 py-3 mb-5">
        <div className="text-xs text-muted-foreground mb-0.5">Flask Code (auto-generated)</div>
        <div className="font-mono text-lg font-bold">{flaskCode}</div>
      </div>
      ...
      <div className="space-y-5">
          ...
          {extractGrams > 0 && (
            <div className={cn(
              ...
              <span className="font-mono font-bold">
                {insufficientStock ? 'Insufficient' : `${remaining.toFixed(2)}g`}
              </span>
            </div>
          )}
        </div>
        ...
        <Button
          ...
          size="lg"
        >
          {extractMutation.isPending ? 'Extracting...' : `Extract ${extractGrams > 0 ? extractGrams.toFixed(2) + 'g' : ''}`}
        </Button>
      </div>
    </div>
  );
